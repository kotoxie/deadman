...
export function requireAuth(req, res, next) {
  if (req.session && req.session.authenticated) {
    ...
    // Idle timeout check
    const idleMinutes = parseInt(Setting.get('session_idle_timeout_minutes')) || 60;
    const idleMs = idleMinutes * 60 * 1000;
    const now = Date.now();
    if (req.session.lastActivity && (now - req.session.lastActivity) > idleMs) {
      return req.session.destroy((err) => {
        if (err) logger.error('Session destroy failed:', err);
        AuditLog.log('Session expired: idle timeout', 'auth', 'info', null, req.ip);
        res.status(401).json({ error: 'Session expired due to inactivity. Please log in again.', idleTimeout: true });
      });
    }
...
export function checkAuth(req, res) {
  ...
  res.json({
    ...
    setupRequired,
    encryptionKeyAutoGenerated: config.encryptionKeyAutoGenerated,
    idleTimeoutMinutes: parseInt(Setting.get('session_idle_timeout_minutes')) || 60,
